...
def create_train_transforms(cfg):
    ...
    additional_targets=dict((f'image{i}', 'image') for i in range(max(cfg.sequence_len_trn, cfg.sequence_len_val))))
...
batch = [self.__getitem__(i) for i in range(0, 300, 70)]
...
class CustomDataset(Dataset):
    ...
    def __init__(self, df, cfg, aug=None, mode="train"):
        ...
        if mode=='train':
            ...
            sequence_len = self.sequence_len =  cfg.sequence_len_trn
        else:
            self.transform = create_valid_transforms(cfg)
            sequence_len = self.sequence_len =  cfg.sequence_len_val

        self.df = self.df.reset_index(drop = True)
        self.weights = 1 / (self.df.groupby('StudyInstanceUID')['C1'].transform(len) - sequence_len)
        self.df['slice_number_start'] = \
            self.df.groupby('StudyInstanceUID')['slice_number'].transform(max) - (sequence_len+1)
        ...
        self.df['Cct'] = (self.bbdf.slnum_to-self.bbdf.slnum_from).loc[self.df.StudyInstanceUID].values
        every_n = (2000/self.df['Cct']).round().astype(int)
        
        self.df['val_pos'] = (self.df['slice_number'] % (sequence_len//6)) == 0 
        self.df['trn_pos'] = (self.df['slice_number'] % (every_n)) == 0 
    ...
    def __getitem__(self, idx):
        ...
        #if self.mode == 'train':
        dfseq = dfseq.loc[start_pos:]
        if self.mode == 'train':
            if self.cfg.random_drop_slices > np.random.random():
                fwd_seq_len = len(dfseq)
                max_drop = min(self.cfg.random_drop_max, fwd_seq_len // self.sequence_len)
                if max_drop>1:
                    drop_dicoms = np.random.randint(2, max_drop+1)
                    dfseq = dfseq.iloc[np.random.randint(drop_dicoms)::drop_dicoms]
        dfseq = dfseq.iloc[:self.sequence_len]
        ...
        slice_stats = torch.arange(start_pos, (start_pos+self.sequence_len))
        ...
        # print(len(out['image']))        
        if (len(out['image'])!=self.sequence_len//3):
            return None
