...
def collate_fn(batch):
    ...
    cols = 'image label_seg label_frac slice_number StudyUID slice_stats'.split()
...
batch = [self.__getitem__(i) for i in range(0, 1700, 700)]
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        out['label_seg'] = torch.tensor(dfseq[self.cfg.target_seg].values)
        out['label_frac'] = torch.tensor(dfseq[self.cfg.target_frac].values)
        ...
        if zdirn>0:
            out['label_seg'] = torch.flip(out['label_seg'], [0])
            out['label_frac'] = torch.flip(out['label_frac'], [0])
